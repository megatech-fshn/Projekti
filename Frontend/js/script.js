...
const API_BASE_URL = "http://localhost/TelekomAlbania/backend/api";

async function apiRequest(endpoint, options = {}) {
  const token = localStorage.getItem("telecomToken") || sessionStorage.getItem("telecomToken") || "";
  const headers = {
  ...
  if (!response.ok || data.success === false) {
    throw new Error(data.message || "Kërkesa dështoi. Provo përsëri.");
  }
...
function normalizeBackendUser(rawUser = {}) {
  const profile = rawUser.profile || rawUser;
  return {
    id: rawUser.id,
    email: rawUser.email,
    password: rawUser.password || "",
    role: rawUser.role || "user",
    status: rawUser.status || "Aktiv",
    profile: {
      ...
      customerType: profile.customer_type || profile.customerType || "Individual"
    }
  };
}

function syncBackendUserToLocalStorage(rawUser, password = "") {
  const nextUser = normalizeBackendUser({ ...rawUser, password });
  const users = getUsers();
  const index = users.findIndex(user => String(user.email || "").toLowerCase() === String(nextUser.email || "").toLowerCase());
...
function storeBackendSession(data, rememberUser = false, password = "") {
  const token = data.token || "";

  clearAuth();

  localStorage.removeItem("telecomToken");
  sessionStorage.removeItem("telecomToken");

  const user = syncBackendUserToLocalStorage(data.user, password);
  ...
  if (rememberUser) {
    localStorage.setItem("telecomToken", token);
    localStorage.setItem(APP_STORAGE.auth, authPayload);
  } else {
    sessionStorage.setItem("telecomToken", token);
...
function initForms() {
  ...
  handleCustomValidation(loginForm, async () => {
    ...
    try {
      const data = await apiRequest("/auth/login.php", {
        method: "POST",
        body: JSON.stringify({ email, password, remember: rememberUser })
      });

      storeBackendSession(data, rememberUser, password);
      if (window.toast) window.toast.show(data.message || "Login u krye me sukses.", "success");
      redirectAfterLogin();
    } catch (error) {
      if (loginError) {
        loginError.textContent = error.message || "Email ose password i pasaktë.";
        loginError.classList.remove("d-none");
      } else if (window.toast) {
        window.toast.show(error.message || "Login dështoi.", "error");
      } else {
  ...
  handleCustomValidation(registerForm, async () => {
    const firstName = normalizeTextSpaces(document.getElementById("firstName")?.value || "");
    const lastName = normalizeTextSpaces(document.getElementById("lastName")?.value || "");
    const email = document.getElementById("registerEmail")?.value.trim().toLowerCase() || "";
    const phone = normalizePhoneStorage(document.getElementById("phoneNumber")?.value || "");
    ...
    try {
      ...
      syncBackendUserToLocalStorage(data.user, password);
      ...
      if (window.toast) window.toast.show(data.message || "Llogaria u krijua me sukses!", "success");
      else alert(data.message || "Llogaria u krijua me sukses.");

      window.location.href = "login.html";
    } catch (error) {
      if (registerError) {
        registerError.textContent = error.message || "Regjistrimi dështoi.";
        registerError.classList.remove("d-none");
      } else if (window.toast) {
        window.toast.show(error.message || "Regjistrimi dështoi.", "error");
      } else {
        alert(error.message || "Regjistrimi dështoi.");
      }
    } finally {
      if (submitBtn) {
        submitBtn.disabled = false;
        submitBtn.textContent = "Krijo Llogari";
      }
    }
  });
}
...
async function renderServices() {
  ...
  function getServiceFeatures(service) {
    ...
    return ["Detajet e paketës nuk janë specifikuar."];
  }

  function servicePriceLabel(service) {
    const price = Number(service.price || 0);
    return price === 0 ? "Falas" : formatAmount(price);
  }
  ...
  function renderCurrentPlan() {
    const currentPlanName = user.profile?.plan || "No active plan";

    currentPlanCard.innerHTML = `
      <div class="plan-card current-plan">
        <span class="notification-pill">Aktiv</span>
        <h4 class="mb-1">${currentPlanName}</h4>
        <div class="text-muted">Plani aktual i përdoruesit</div>
      </div>
    `;
  }
  ...
  function openPurchaseModal(service) {
    ...
    document.getElementById("serviceSelectedPlan").value = service.id;
    setText("serviceModalPlanName", service.name);
    setText("serviceModalPlanAmount", servicePriceLabel(service));
    setText("serviceModalPlanMeta", `${service.category || "Plan"} • ${serviceMetaLabel(service)}`);
  ...
  servicesGrid.innerHTML = `
    <div class="col-12">
      <div class="text-muted">Duke ngarkuar paketat nga databaza...</div>
    </div>
  `;

  try {
    const data = await apiRequest("/services/list.php");
    backendServices = data.services || [];

    if (!Array.isArray(backendServices) || backendServices.length === 0) {
      servicesGrid.innerHTML = `
        <div class="col-12">
          <div class="empty-row py-4">Nuk ka paketa në databazë.</div>
        </div>
      `;
      return;
    }

    servicesGrid.innerHTML = backendServices.map(service => {
      const features = getServiceFeatures(service);
      const isCurrentPlan = service.name === user.profile?.plan;

      return `
        <div class="col-md-6">
          <div class="plan-card ${isCurrentPlan ? "current-plan" : ""}">
            <span class="notification-pill">${service.category || "Plan"}</span>
            <h5 class="mb-1">${service.name}</h5>
            <div class="plan-price">${servicePriceLabel(service)}</div>
            <div class="text-muted">${serviceMetaLabel(service)}</div>
            ...
            <button 
              class="btn ${isCurrentPlan ? "btn-outline-primary" : "btn-primary"} mt-auto service-select-btn" 
              data-service-id="${service.id}" 
              type="button" 
              ${isCurrentPlan ? "disabled" : ""}
            >
              ${isCurrentPlan ? "Plani Aktual" : "Zgjidh Paketën"}
            </button>
          </div>
        </div>
      `;
    ...
    document.querySelectorAll(".service-select-btn").forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        const service = backendServices.find(item => String(item.id) === String(serviceId));

        if (!service) {
          alert("Paketa nuk u gjet.");
          return;
        }

        openPurchaseModal(service);
      });
    });
    ...
    servicesGrid.innerHTML = `
      <div class="col-12">
        <div class="empty-row py-4">${error.message || "Nuk u ngarkuan paketat."}</div>
      </div>
    `;
  }
  ...
  if (purchaseForm && !purchaseForm.dataset.bound) {
    ...
    handleCustomValidation(purchaseForm, async () => {
      ...
      if (!selectedService) {
        alert("Nuk është zgjedhur asnjë paketë.");
        return;
      }
      ...
      try {
        const result = await apiRequest("/subscriptions/create.php", {
          method: "POST",
          body: JSON.stringify({
        ...
        setText("serviceSuccessAmount", servicePriceLabel(selectedService));
        setText("serviceSuccessMethod", paymentMethod);

        openSimpleModal("serviceSuccessModal");

        if (window.toast) {
          window.toast.show(result.message || "Paketa u aktivizua me sukses.", "success");
        }

        renderServices();

      } catch (error) {
        alert(error.message || "Abonimi dështoi.");
      }
    });
  }
  ...
  document.getElementById("servicePurchaseModal")?.addEventListener("click", event => {
    if (event.target === document.getElementById("servicePurchaseModal")) {
      closeSimpleModal("servicePurchaseModal");
    }
  });
  ...
  document.getElementById("serviceSuccessModal")?.addEventListener("click", event => {
    if (event.target === document.getElementById("serviceSuccessModal")) {
      closeSimpleModal("serviceSuccessModal");
    }
  });
}
