...
--
-- Host: 127.0.0.1
-- Generation Time: May 19, 2026 at 05:55 PM
...
CREATE TABLE `bills` (
  ...
  `status` enum('Paguar','Papaguar','Vonuar') DEFAULT 'Papaguar',
...
CREATE TABLE `notifications` (
  ...
  `type` enum('billing','payment','service','system') DEFAULT NULL,
  `audience` enum('user','all') DEFAULT 'user',
...
CREATE TABLE `services` (
  ...
  `category` enum('Internet','Mobile','Combo','Business','Free') DEFAULT NULL,
...
CREATE TABLE `transactions` (
  ...
  `payment_method` enum('Kartë Krediti','Kartë Debiti','PayPal','Transfertë Bankare') DEFAULT NULL,
  `amount` decimal(10,2) DEFAULT NULL,
  `status` enum('Sukses','Në proces','Dështuar') DEFAULT 'Sukses',
...
CREATE TABLE `users` (
  ...
  `status` enum('Aktiv','Pasiv','Bllokuar') DEFAULT 'Aktiv',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
ALTER TABLE `bills`
  ...
  ADD CONSTRAINT `bills_ibfk_2` FOREIGN KEY (`service_id`) REFERENCES `services` (`id`) ON DELETE SET NULL;
