...
window.toast = (() => {
  ...
  function show(message, type = 'info', duration = 4000) {
    ...
    toast.innerHTML = `
      <span class="tc-toast__icon">${icons[type] || icons.info}</span>
      <span class="tc-toast__msg">${message}</span>
      <button class="tc-toast__close" aria-label="Mbyll njoftimin">×</button>
    `;
    c.appendChild(toast);
    ...
    toast.querySelector('.tc-toast__close').addEventListener('click', dismiss);
...
function initSidebarUserInfo() {
  ...
  try {
    ...
    const fullName = `${user.profile?.firstName || ''} ${user.profile?.lastName || ''}`.trim() || auth.email;
    nameEl.textContent = fullName;
    if (roleEl) roleEl.textContent = auth.role === 'admin' ? '⚙ Admin' : '◉ Klient';
  } catch(e) {}
}
